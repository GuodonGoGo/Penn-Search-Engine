...
public class Row implements Serializable {
  ...
  static String readStringSpace0(InputStream in) throws Exception {
	    byte buffer[] = new byte[16384];
	    int numRead = 0;
	    while (true) {
	      if (numRead == buffer.length)
	      ...
	      int b = in.read();
	      if ((b < 0) || (b == 10)) {
	    	  //System.out.println("row key terminated, current numRead: " + numRead);
	    	  return null;  
	      }
  ...
  public static Row readFrom(InputStream in) throws Exception {
    String theKey = readStringSpace0(in);
    if (theKey == null) {
    ...
    while (true) {
      ...
      byte b = (byte)in.read();
      if (b != ' ')
        throw new Exception("Expecting a space separator after value for key '"+keyOrMarker+"'" + "rowKey: " + theKey + " len: " + len + "  bytesRead: " + bytesRead + " b: " + b + " theValue: " + new String(theValue));
  ...
  public synchronized byte[] contentToByteArray()  {
	    ByteArrayOutputStream baos = new ByteArrayOutputStream();
