...
public class KVSClient implements KVS {
  ...
  class KVSIterator implements Iterator<Row> {
      ...
      try {
        ...
        while (true) {
          ...
          try {
            ...
            // System.out.println("read one row");
            Row r = fill();
            if (r != null) {
              nextRow = r;
              break;
    ...
    synchronized Row fill() {
      try {
        ...
        // if(r == null) {
        // System.out.println("row is null in fill");
        // }
    ...
    public synchronized Row next() {
      if (atEnd)
        return null;
      Row r = nextRow;
      // System.out.println("Row key: " + r.key() + " Row resCode: " +
      // r.get("responseCode"));

      nextRow = fill();

      while ((nextRow == null) && !atEnd) {
        // System.out.println("when nexRow == null");
        currentRangeIndex++;
        openConnectionAndFill();
        // nextRow = fill();
        // System.out.println("row still null");
      }
  ...
  public Row getRow(String tableName, String row) throws IOException {
    ...
    // System.out.println("achive getRow 267");
    // System.out.println("getRow url: " +
    // "http://"+workers.elementAt(workerIndexForKey(row)).address+"/data/"+tableName+"/"+java.net.URLEncoder.encode(row,
    // "UTF-8"));

    // System.out.println("KVS Client test: " +
    // workers.elementAt(workerIndexForKey(row)).address);
    ...
    // System.out.println("achive getRow rescode: " + resp.statusCode());
    if (resp.statusCode() == 404)
      return null;

    byte[] result = resp.body();
    // System.out.println("achive getRow res.body: " + resp.body());
    try {
