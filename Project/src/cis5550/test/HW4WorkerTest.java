  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("read-id"))
      try {
        ...
        if (r.statusCode != 200)
          testFailed("The coordinator returned a " + r.statusCode
              + " response to our GET /workers, but we were expecting a 200 OK. Here is what was in the body:\n\n"
              + dump(r.body));
        ...
        if ((pieces.length < 2) || !pieces[1].contains(id))
          testFailed(
              "The coordinator did return one worker in its response to our GET /workers, but the ID we had written to __worker/id ("
                  + id + ") did not appear. Here is what the coordinator sent:\n\n" + dump(r.body)
                  + "\nMaybe the worker didn't read the ID frome the 'id' file in the storage directory - or maybe you didn't provide the correct path when you started the worker? It was supposed to be "
                  + f.getAbsolutePath()
                  + ". Also, remember to start the worker AFTER the test suite (when you are prompted to hit Enter); if it is already running, the test suite will overwrite the ID and then expect to see the new value, but the worker won't see it.");
        s.close();
    ...
    if (tests.contains("put"))
      try {
        ...
        if (!r.body().equals("OK"))
          testFailed("The server did return a 200 status code to our " + req
              + ", but it was supposed to return 'OK', and it didn't. Here is what was in the body instead:\n\n"
              + dump(r.body));
        s.close();
        testSucceeded();
      } catch (Exception e) {
    ...
    if (tests.contains("putget"))
      try {
        ...
        if (!r.body().equals(data))
          testFailed("The server did return a 200 status code to our " + req
              + ", but it was supposed to return the string we had PUT in earlier (" + data
              + "), and it didn't. Here is what was in the body instead:\n\n" + dump(r.body));
        s.close();
    ...
    if (tests.contains("overwrite"))
      try {
        ...
        if (r.body().equals(data1))
          testFailed("The server did return a 200 status code to our " + req
              + ", but it was supposed to return the new value we had PUT (" + data2
              + "), and it returned the old value (" + data1 + ") instead.");
        if (!r.body().equals(data2))
          testFailed("The server did return a 200 status code to our " + req
              + ", but it was supposed to return the second string we had PUT in (" + data2
              + "), and it didn't. Here is what was in the body instead:\n\n" + dump(r.body));
        s.close();
    ...
    if (tests.contains("search"))
      try {
        ...
        Socket s = openSocket(8001);
        PrintWriter out = new PrintWriter(s.getOutputStream());
        String cell = "/search?query=galaxy";
        String req = "GET " + cell;
        out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
        out.flush();
        Response r = readAndCheckResponse(s, "response");
    ...
    if (tests.contains("stress"))
      try {
        ...
        for (int i = 0; i < numThreads; i++) {
          t[i] = new Thread("Client thread " + i) {
            public void run() {
                    ...
                    if (random(0, 1) == 0) {
                      ...
                      if (expectedAtLeastVersion > 0) {
                        if (r.statusCode != 200) {
                          ...
                          System.err.println("GET returned code " + r.statusCode + " for row: " + row + ", body: "
                              + new String(r.body));
                        } else {
                          ...
                          if (v < expectedAtLeastVersion) {
                            testsFailed.increment();
                            System.err.println("GET returned version " + v + " for row " + row
                                + ", but that has been overwritten; we expected at least version "
                                + expectedAtLeastVersion);
                          }
                          if (!dat.get(row + "-" + v).equals(val)) {
                            testsFailed.increment();
                            System.err.println("GET returned value " + val + " for row " + row
                                + ", but we expected version " + v + " to be " + dat.get(row + "-" + v).equals(val));
                          }
                        }
                      }
                    } else {
                      ...
                      out.print("PUT /data/" + tabname + "/" + row
                      ...
                      if (r.statusCode != 200) {
                        testsFailed.increment();
                        System.err.println(
                            "PUT returned code " + r.statusCode + " for row: " + row + ", body: " + new String(r.body));
                      }
                      synchronized (ver) {
        ...
        if (numFail > 0)
          testFailed("Looks like " + numFail + " of the " + testsTotal
              + " requests failed. Check your code for concurrency issues! (This is a very difficult test case, so you may want to leave it until the very end, when your implementation passes all the other tests.)");
        if (reqPerSec < 1000)
          testFailed("Looks like your solution handled " + reqPerSec
              + " requests/second; it should be at least 1,000. Try disabling any debug output?  (This is a very difficult test case, so you may want to leave it until the very end, when your implementation passes all the other tests.)");
