...
class Coordinator extends cis5550.generic.Coordinator {
  ...
  public static String kvsAddress;
  public static KVSClient kvs;
  public static Vector<String> kvsWorkers = new Vector<String>(); 
    ...
    post("/submit", (request,response) -> {
      ...
      String resultKvsWorkers = new String(HTTP.doRequest("GET", "http://"+kvsAddress+"/workers", null).body());
      String[] lines = resultKvsWorkers.split("\r\n");
      for(int i = 1; i < lines.length; i++) {
    	  kvsWorkers.add(lines[i]);
      }
      ...
      for (int i=0; i<getWorkers().size(); i++) {
    	/****************/
    	String[] strs = getWorkers().get(i).split(":");
		String id = strs[0];
		String ip = strs[1];
		String port = strs[2];
		/******************/
		
        final String url = "http://"+ ip + ":" + port +"/useJAR";
        final int j = i;
      ...
      if(fc.outputBody == "") {
    	  return "There was no output";
      }else {
    	  return fc.outputBody;
      }
    });
